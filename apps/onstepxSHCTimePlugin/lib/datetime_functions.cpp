...
void date_toString(DateInfo dateInfo, char buffer[], int bufferSize) {
    ...
    snprintf(buffer, sizeof(buffer), "%02ld/%02ld/%02ld", dateInfo.month, dateInfo.day, dateInfo.year);
};

void time_12h_toString(TimeInfo timeInfo, char buffer[], int bufferSize) {
    memset(buffer, '\0', bufferSize);
    snprintf(buffer, sizeof(buffer), "%02ld:%02ld:%02ld", timeInfo.hour, timeInfo.minutes, timeInfo.seconds);
};
...
void time_24h_toString(TimeInfo timeInfo, char buffer[], int bufferSize) {
    ...
    snprintf(buffer, sizeof(buffer), "%02ld:%02ld:%02ld", hour, timeInfo.minutes, timeInfo.seconds);
};
